...
export function TechnologiesSection() {
  ...
  useEffect(() => {
    console.log('[Website-Services] TechnologiesSection mounted');
    return () => console.log('[Website-Services] TechnologiesSection unmounted');
  }, []);
  ...
  return (
    <section className="py-20 relative" id="technologies" role="region" aria-labelledby="ws-tech-heading">
      {/* Background pattern */}
        ...
        <div className="mb-12 flex justify-center">
          <div className="flex flex-wrap justify-center gap-2 p-1 bg-gray-100 dark:bg-gray-800/50 rounded-full max-w-4xl overflow-x-auto">
            {(technologies || []).map((tech) => (
              <button
                key={tech.category}
                onClick={() => { console.log('[Website-Services] Tech category', { category: tech.category }); setActiveCategory(tech.category); }}
                className={cn(
                ...
                aria-pressed={activeCategory === tech.category}
              >
                {tech.category}
              </button>
            ))}
