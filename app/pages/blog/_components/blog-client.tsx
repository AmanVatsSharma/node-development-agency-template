...
export function BlogClient({ initialPosts }: { initialPosts: BlogPost[] }) {
  ...
  const [blogData, setBlogData] = useState<BlogPostResponse | null>(
    initialPosts.length > 0
      ? { posts: initialPosts, pagination: { total: initialPosts.length, page: 1, pageSize: postsPerPage, totalPages: 1, hasMore: false } }
      : null
  );
  ...
  useEffect(() => {
    // Only fetch from client when user interacts (search/filter/page change)
    if (!searchQuery && selectedCategory === 'all' && currentPage === 1) return;
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
                ...
                >
                  ...
                  <div className="p-5 sm:p-8">
                    <div className="flex items-center mb-4">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">{formatDate(post.publishedAt)}</span>
                      <span className="mx-2 text-gray-400">•</span>
                      <span className="text-sm text-gray-600 dark:text-gray-400">{post.readTime} min read</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-5">
                      {post.tags.map((tag, idx) => (
                        <span key={idx} className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded-full text-xs">{tag}</span>
                      ))}
                    </div>
                    <Link href={`/pages/blog/${post.slug}`} className="inline-flex items-center font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                      Read article
            ...
            {categories.map((category) => (
              <button
                ...
                className={`px-4 py-2.5 rounded-full text-sm whitespace-nowrap transition-colors ${
                  selectedCategory === category.id ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'
                }`}
              >
                {category.name}
              </button>
                        ...
                        <div className="p-5 sm:p-6">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              ...
                              <span className="text-xs text-gray-600 dark:text-gray-400">{post.readTime} min read</span>
                            </div>
                            <Link href={`/pages/blog/${post.slug}`} className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                              Read more →
                            </Link>
                          </div>
                        </div>
                  ...
                  <div className="flex justify-center mt-8">
                    <nav className="flex space-x-2 overflow-x-auto hide-scrollbar pb-1">
                      <button
                        ...
                        className={`px-3 py-1 rounded-md ${currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                      >
                        ← Prev
                      </button>
                      {Array.from({ length: blogData.pagination.totalPages }, (_, i) => i + 1).map((number) => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-md ${currentPage === number ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                        >
                          {number}
                        </button>
                      ))}
                      <button
                        onClick={() => setCurrentPage(p => Math.min(p + 1, blogData.pagination.totalPages))}
                        disabled={currentPage === blogData.pagination.totalPages}
                        className={`px-3 py-1 rounded-md ${currentPage === blogData.pagination.totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                      >
                        Next →
                      </button>
                    </nav>
      ...
      <div className="bg-blue-600 compact-main-section text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <Link href="/pages/contact" className="px-8 py-3 bg-white text-blue-600 hover:bg-gray-100 rounded-2xl sm:rounded-full font-medium inline-block transition-colors">
            Get in Touch
          </Link>
        </div>
      </div>
