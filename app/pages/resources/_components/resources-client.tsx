...
export function ResourcesClient({ initialResources }: { initialResources: Resource[] }) {
  ...
  const [resourcesData, setResourcesData] = useState<ResourcesResponse | null>(
    initialResources.length > 0
      ? { resources: initialResources, pagination: { total: initialResources.length, page: 1, pageSize, totalPages: 1, hasMore: false } }
      : null
  );
  const [page, setPage] = useState(1);

  const [email, setEmail] = useState('');
  const [newsletterStatus, setNewsletterStatus] = useState<{ submitting: boolean; success: boolean; error: string | null }>({
    submitting: false,
    success: false,
    error: null,
  });
  ...
  useEffect(() => {
    // Skip initial fetch — initialResources covers page 1 defaults
    if (!searchQuery && selectedType === 'all' && page === 1) return;
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEmail('');
      setNewsletterStatus({ submitting: false, success: true, error: null });
      setTimeout(() => setNewsletterStatus(prev => ({ ...prev, success: false })), 5000);
    } catch {
        ...
        <div className="container mx-auto px-4">
          <div className="touch-chip-row py-4 md:justify-center md:overflow-visible">
            <button
              ...
              className={`px-4 py-2 rounded-full whitespace-nowrap ${selectedType === 'all' ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
            >
              All Resources
            </button>
            {(Object.keys(resourceTypeIcons) as Resource['type'][]).map((type) => (
              <button
                key={type}
                onClick={() => setSelectedType(type)}
                className={`px-4 py-2 rounded-full flex items-center whitespace-nowrap ${selectedType === type ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
              >
                ...
                >
                  ...
                  <div className="relative z-10 p-5 sm:p-8 flex flex-col md:flex-row items-start gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="text-xl sm:text-2xl font-bold text-white mb-3 text-balance">{resource.title}</h3>
                  ...
                  >
                    <div className="p-5 sm:p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${resourceTypeColors[resource.type].bg} ${resourceTypeColors[resource.type].text}`}>
                        <span className="mr-1.5">{resourceTypeIcons[resource.type]}</span>
                        {resource.type.charAt(0).toUpperCase() + resource.type.slice(1)}
                      </span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(resource.publishedAt)}</span>
                    </div>
                    <div className="p-5 sm:p-6">
                      <h3 className="text-lg sm:text-xl font-bold mb-3 text-gray-800 dark:text-white text-balance">{resource.title}</h3>
                      <p className="text-gray-600 dark:text-gray-300 text-sm mb-6">{resource.description}</p>
                      <a
                        href={resource.downloadUrl}
              ...
              <div className="flex justify-center mt-12">
                <nav className="flex space-x-2 overflow-x-auto hide-scrollbar pb-1">
                  <button
                    ...
                    disabled={page === 1}
                    className={`px-4 py-2 rounded-md ${page === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                    Previous
                  </button>
                  {Array.from({ length: resourcesData.pagination.totalPages }, (_, i) => i + 1).map((number) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-md ${page === number ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                    >
                      {number}
                    </button>
                  ))}
                  <button
                    ...
                    className={`px-4 py-2 rounded-md ${page === resourcesData.pagination.totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                    Next
                  </button>
                </nav>
