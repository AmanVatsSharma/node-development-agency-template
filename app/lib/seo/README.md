...
- metadata helper invariants in `app/lib/seo/metadata.ts` (`normalizeMetadataPath` for canonical path hygiene and `normalizeMetadataKeywords` for trimmed case-insensitive deduplication before `buildPageMetadata` output)
