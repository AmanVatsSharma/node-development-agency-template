...
export function FAQSection() {
  ...
  useEffect(() => {
    console.log('[Website-Development] FAQSection mounted');
    return () => console.log('[Website-Development] FAQSection unmounted');
  }, []);
  ...
  return (
    <section className="py-20 relative overflow-hidden" id="faq" role="region" aria-labelledby="wd-faq-heading">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mx-auto">
          {(faqs || []).map((faq) => (
            ...
            >
              <button
                onClick={() => { console.log('[Website-Development] FAQ toggle', { id: faq.id }); toggleAccordion(faq.id); }}
                className="flex items-center justify-between w-full p-5 text-left"
                aria-expanded={openItem === faq.id}
                aria-controls={`wd-faq-panel-${faq.id}`}
              >
              ...
              {openItem === faq.id && (
                <div id={`wd-faq-panel-${faq.id}`} className="p-5 pt-0 text-muted-foreground">
                  {faq.answer}
                </div>
              )}
            </div>
