...
export function PricingSection() {
  const handleCTAClick = (plan: string) => {
    console.log(`[Ecommerce-Pricing] CTA clicked: ${plan}`);
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'ecommerce_pricing_cta_click', {
        plan: plan,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
            Hire Ecommerce Google Ads Expert Pricing & Packages
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Choose the perfect e-commerce plan for your online store. All plans include our 6.8× ROAS guarantee.
          </p>
        </motion.div>
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative p-8 rounded-2xl ${
                plan.popular
                  ? 'bg-gradient-to-br from-orange-600 to-red-700 text-white shadow-2xl scale-105'
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className={`text-sm mb-4 ${plan.popular ? 'text-orange-100' : 'text-gray-600'}`}>
                  {plan.description}
                </p>
                ...
                {plan.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start gap-3">
                    <Check className={`w-5 h-5 mt-0.5 flex-shrink-0 ${
                      plan.popular ? 'text-green-400' : 'text-orange-600'
                    }`} />
                    <span className={`text-sm ${plan.popular ? 'text-orange-100' : 'text-gray-600'}`}>
                      {feature}
                    </span>
                  </li>
                ))}
        ...
        >
          <p className="text-gray-600 mb-4">
            All plans include our 6.8× ROAS guarantee and 30-day money-back guarantee.
          </p>
